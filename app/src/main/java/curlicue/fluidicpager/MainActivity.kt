...
class MainActivity : AppCompatActivity() {
    ...
    lateinit var viewPagerVp: ViewPager
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewPagerVp.setPageTransformer(true, FluidicPageTransformer(R.id.fragmentPage_content_Fl))
    }
...
class FluidicPageTransformer(private var pageRootId: Int) : ViewPager.PageTransformer {

    override fun transformPage(view: View, position: Float) {
        val pageRootView = view.findViewById<View>(pageRootId)
        when {
            ...
            position <= 0 -> { // ]-1,0]
                ...
                pageRootView.translationX = -(position * pageRootView.width)
            }
            position <= 1 -> { // ]0,1]
